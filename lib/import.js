    ...
    try {
      ...
      if ('response.status' in err) {
        switch (err.response.status) {
          case 401:
            console.error('searchTag', err.message);
            try {
              authToken = await req.getToken();
            } catch(err) {
              logger.error('searchTag', err.message);
            }
            continue;

          case 400:
          case 408:
            logger.error(pageNum, 'searchTag', err.message);
            pageNum++;
            continue;
      ...
      for (let i = 0; i < batchRequest.length; i = i + 25) {
        try {
          ...
          if (reqSlice.length > 0) {
            await ddb.itemsWrite(tabName, reqSlice);
          }
        } catch(err) {
...
if (creatorType !== undefined && creatorType) {
  ...
  console.info('Usage: node ' + path.basename(process.argv[1]) + ' <creatorType(Artist/Studio)>');
}
