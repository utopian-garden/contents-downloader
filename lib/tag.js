...
// ライブラリ
import { getToken, v2SearchPost, tagPut } from './req.js';
...
const tagPosts = async (procType, tagKey, searchKey, pageFeed) => {
  ...
  while (loop) {
    ...
    // 検索リクエスト
    try {
      searchRes = await v2SearchPost(searchKey, pageNum, next, authToken);

      await waitTimer(shortPoll);
    } catch (err) {
      if (!err.response) {
        logger.error('v2SearchPost', tagKey, searchKey, pageNum, err.code);
        await waitTimer(shortPoll);
        continue page_loop;
      }

      logger.error(
        'v2SearchPost', tagKey, searchKey, pageNum, err.response.status
      );
