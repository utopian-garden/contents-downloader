...
const quePosts = async (dlType) => {
  ...
  while (loop) {
    ...
    while (loop) {
      ...
      if (searchMsgBody) {
        try {
          await sqs.sendMsg(queUrl.searchQueUrl, JSON.stringify(searchMsgBody));
        } catch (err) {
      ...
      if (rmMsgBody) {
        try {
          await sqs.sendMsg(queUrl.removeQueUrl, JSON.stringify(rmMsgBody));
        } catch (err) {
          logger.error('sendMsg', queUrl.removeQueUrl, pageNum, err.message);
        }
      }
