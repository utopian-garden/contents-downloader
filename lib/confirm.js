...
exports.dbConfirm = async (confirmType) => {
  ...
  switch (confirmType) {
    case dlTab:
      ...
      conSubDirs.push(appConfig.fs.dlImageOkDir);
      conSubDirs.push(appConfig.fs.dlVideoOkDir);
      break;
    ...
    case igTab:
      ...
      conSubDirs.push(appConfig.fs.igNgDir);
      break;
  }
        ...
        for (let item of searchRes) {
          for (let tag of item.tags) {
            ...
            if (!tagKey.match(/^[\x20-\x7e]*$/) && tag.name_ja === tagKey &&
              ...
              for (let conSubDir of conSubDirs) {
                const enSubDir = path.join(conSubDir, sanitize(tag.name_en));
                const jaSubDir = path.join(conSubDir, sanitize(tag.name_ja));
                if (fs.pathExistsSync(jaSubDir)) {
                  fs.moveSync(jaSubDir, enSubDir, { overwrite: true });
                }
              }
            ...
            if ((tag.name_en === tagKey || tag.name_ja === tagKey) &&
              ...
              for (let conSubDir of conSubDirs) {
                if (tag.name_en) {
                  const enSubDir = path.join(conSubDir, sanitize(tag.name_en));
                  if (fs.pathExistsSync(enSubDir)) {
                    fs.removeSync(enSubDir);
                  }
                }
