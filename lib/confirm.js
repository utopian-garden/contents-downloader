...
const { postSearchUrl, conSearchParam } = appConfig.req.search;
...
const dbConfirm = async (confirmType) => {
  ...
  for (let tabItem of tabItems) {
    ...
    while (2 > pageNum) {
      ...
      let searchRes;
      try {
        searchRes = await searchPost(tagKey, pageNum, conSearchParam, authToken);
      ...
      if (searchRes) {
        if (searchRes.length === 0) {
          logger.debug(
            'nothing', tagKey, postSearchUrl + encodeURIComponent(tagKey) + conSearchParam + pageNum
          );
          break;
        }
