      ...
      while (true) {
        ...
        try {
          childProcess.execFileSync('node', ['./lib/req-search-fork.js',
              encodeURIComponent(tagKey), pageNum, searchParam, authToken]);
        } catch(err) {
          if (err.status) {
            switch (err.status) {
              case 19190721:
        ...
        try {
          ...
          //logger.debug('searchPost', tagKey, pageNum, searchRes.length);
        } catch(err) {
          if (err.response) {
        ...
        */
        ...
        let searchRes;
        try {
          searchRes = await sqs.recvMsg(searchQueUrl);
        } catch(err) {
