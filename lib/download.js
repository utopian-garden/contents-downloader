...
// ライブラリ
import { dlContent } from './req-dl-fork.js';
...
const dlPosts = async () => {
  ...
  while (loop) {
    ...
    for (let post of dlItems) {
      ...
      const samplelUrl = post.sample_url;
      ...
      try {
        dlContent(filePath, fileUrl, refererUrl);
      } catch (err) {
        isFailed = true;
      ...
      if (isFailed) {
        try {
          dlContent(samplePath, samplelUrl, refererUrl);
        } catch (err) {
          logger.error('dlSampleFork', err.message);
        }
      }
    }
  }
