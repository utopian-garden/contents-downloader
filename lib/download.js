...
exports.dlPosts = async () => {
  ...
  while (true) {
    ...
    for (let item of searchRes) {
      ...
      const curLast = Number(searchQueRes.curLast);
      if (curLast >= postId) {
        break;
      }
      ...
      const saniFileName = sanitize(fileName);
      ...
      for (let tag of item.tags) {
        ...
        if (ngText.includes(tag.name_en)) {
          if ((dlImageFiles.includes(fileName) ||
            ...
            fs.removeSync(path.join(dlImageDir, saniFileName));
            fs.removeSync(path.join(dlImageDir, "image", saniFileName));
            fs.removeSync(path.join(dlImageDir, "image", "ok", saniFileName));
            fs.removeSync(path.join(dlImageOkDir, saniFileName));
            fs.removeSync(path.join(dlImageOkDir, "image", saniFileName));
            fs.removeSync(path.join(dlImageOkDir, "image", "ok", saniFileName));

            fs.removeSync(path.join(dlVideoDir, saniFileName));
            fs.removeSync(path.join(dlVideoDir, "video", saniFileName));
            fs.removeSync(path.join(dlVideoDir, "video", "ok", saniFileName));
            fs.removeSync(path.join(dlVideoOkDir, saniFileName));
            fs.removeSync(path.join(dlVideoOkDir, "video", saniFileName));
            fs.removeSync(path.join(dlVideoOkDir, "video", "ok", saniFileName));
          }
