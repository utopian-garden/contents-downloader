...
const dlPosts = async (dlType) => {
  ...
  while (loop) {
    ...
    for (let post of dlItems) {
      const { id: postId, file_url: fileUrl, created_at: createdAt } = post;
      const { s: createTime } = createdAt;

      // ファイル名の整形
      const fileExt = (fileUrl.match(/.*\.(.*)\?.*/))[1];
      const fileName = createTime + '_' + postId + '.' + fileExt;
      ...
      while (remainRetry > 0) {
        try {
          ...
          if (err.code === 'ERR_NON_2XX_3XX_RESPONSE') {
            ...
            // それ以外のエラーはリトライ
            remainRetry = 3;
            //logger.error('dlContent', filePath, err.message);
            await waitTimer(shortPoll);
            continue dl_loop;
          }
        }
