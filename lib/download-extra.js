...
const extraDlPosts = async (dlType) => {
  ...
  while (loop) {
    ...
    while (dlItems.length < 10) {
      ...
      try {
        searchRes = await searchPost(tagKey, pageNum, searchParam, authToken);
      ...
      for (let post of searchRes) {
        ...
        const { id: postId, tags, file_url: fileUrl } = post;
          ...
          try {
            ...
            if (err.code === 'ERR_NON_2XX_3XX_RESPONSE') {
              switch (err.response.statusCode) {
                ...
                case 503:
                  remainRetry -= 1;
                  logger.error('dlContent', filePath, fileUrl, refererUrl, err.response.statusCode);
                  await waitTimer(shortPoll);
                  continue dl_loop;
