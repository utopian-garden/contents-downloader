...
exports.extraDlPosts = async (dlType) => {
  ...
  while (true) {
    ...
    while (searchRes.length < 10) {
      ...
      // Tag テーブルの post に追加する postid を配列に追加する
      await response.map(item => {
        ...
        if (!ngText.includes(item.name_en) &&
            !ignoreExts.includes(fileExt) &&
            searchRes.length < 10) {

          searchRes.push(item);
          postArray.push(item.id);
        }
      });
