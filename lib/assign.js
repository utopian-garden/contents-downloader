...
const { chanSearchUrl, chanSearchParam } = appConfig.req.search;
  ...
  for (let param of params) {
    ...
    for (let targetTag of targetTags) {
      ...
      if (fixedTargetTag) {
        ...
        logger.debug(
          toTable, chanSearchUrl + targetTag + chanSearchParam, path.join(targetDir, targetTag)
        );
      }
    }
    ...
    for (let oppoTag of oppoTags) {
      ...
      if (wrongOppoTag) {
        ...
        logger.debug(
          fromTable, chanSearchUrl + oppoTag + chanSearchParam, path.join(oppoDir, oppoTag)
        );
      }
    }
  }
