...
// ライブラリ
import { getToken, v2SearchPost, postVote } from './req.js';
...
const votePosts = async (searchKey) => {
  ...
  while (loop) {
    ...
    try {
      ...
      if (!err.response) {
        logger.error('v2SearchPost', searchKey, pageNum, err.code);
        await waitTimer(shortPoll);
        continue page_loop;
      }

      logger.error(
        'v2SearchPost', searchKey, pageNum, err.response.status
      );
