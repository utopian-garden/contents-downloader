...
app.get(apiUri['chkDbUri'], async (req, res) => {
  ...
  try {
    ...
    const artItems = await ddb.queryItem(artTable, tagAttr, reqTag);
    const stuItems = await ddb.queryItem(stuTable, tagAttr, reqTag);
    let resTable;

    // item が入っている DB を判定
    if (dlItems.Items.length > 0) {
      resTable = dlTable;
    } else if (igItems.Items.length > 0) {
      resTable = igTable;
    } else if (artItems.Items.length > 0) {
      resTable = artTable;
    } else if (stuItems.Items.length > 0) {
      resTable = stuTable;
    } else {
      resTable = 'None';
    }
...
app.post(apiUri['addItemUri'], async (req, res) => {
  ...
  switch (reqTable) {
    ...
    case igTable:
      ...
      const tagImageNgDir = path.join(dlImageNgDir, sanitize(reqTag));
      const tagVideoDir = path.join(dlVideoDir, sanitize(reqTag));
      const tagVideoOkDir = path.join(dlVideoOkDir, sanitize(reqTag));
      const tagVideoNgDir = path.join(dlVideoNgDir, sanitize(reqTag));
