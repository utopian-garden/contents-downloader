...
const {
  ...
  v2SearchUrl, v2SearchParam, v2SearchParamNext, refererUrl
...
// ポストの検索 (API v2)
const v2SearchPost = async (tagKey, pageNum, next, authToken) => {
  let searchUrl = v2SearchUrl + encodeURIComponent(tagKey) + v2SearchParam + pageNum;
  if (next) {
    searchUrl += v2SearchParamNext + next;
  }
  const searchOptions = {
    method: 'GET',
    url: searchUrl,
...
export { getToken, searchPost, searchTag, dlContent, tagPut, searchRate, v2SearchPost, postVote };
