...
// ポストの検索
exports.searchPost = async (tagKey, pageNum, searchLimit, authToken) => {
  const searchUrl = appConfig.req.search.searchUrl + tagKey +
      appConfig.req.search.searchParam1 + searchLimit +
      appConfig.req.search.searchParam2 + pageNum;
  const userAgent = appConfig.req.common.userAgent;
  const searchOptions = {
    method: 'GET',
    uri: searchUrl,
