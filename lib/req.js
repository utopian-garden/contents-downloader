...
const {
  rateSearchUrl, tagSearchUrl, postSearchUrl, postSearchParam, postSearchParamNext, refererUrl
...
// ポストの検索
const tagSearchPost = async (tagKey, pageNum, next, accessToken) => {
  let url = tagSearchUrl + encodeURIComponent(tagKey) + postSearchParam + pageNum;
  if (next) {
    url += postSearchParamNext + next;
  }
  const searchOptions = {
...
export { dlContent, tagSearchPost, tagPut, searchRate, searchPost, postVote, refreshTokens };
